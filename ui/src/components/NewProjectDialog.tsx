...
export function NewProjectDialog() {
  ...
  const deriveWorkspaceNameFromPath = (value: string) => {
    ...
    return segments[segments.length - 1] ?? "Folder local";
  };
  ...
  async function handleSubmit() {
    ...
    if (localPath && !isAbsolutePath(localPath)) {
      setWorkspaceError("Folderul local trebuie sa fie o cale absoluta completa.");
      return;
    }
    if (repoUrl && !looksLikeRepoUrl(repoUrl)) {
      setWorkspaceError("Repo-ul trebuie sa foloseasca un URL valid de GitHub sau GitHub Enterprise.");
      return;
    }
      ...
      >
        ...
        <div className="px-4 pt-3 pb-3 space-y-3 border-t border-border">
          <div>
            <div className="mb-1 flex items-center gap-1.5">
              <label className="block text-xs text-muted-foreground">URL Repo</label>
              ...
              <Tooltip delayDuration={300}>
                ...
                <TooltipContent side="top" className="max-w-[240px] text-xs">
                  Leaga un repository GitHub ca agentii sa poata clona, citi si trimite cod pentru acest proiect.
                </TooltipContent>
              </Tooltip>
            </div>
            <input
          ...
          <div>
            <div className="mb-1 flex items-center gap-1.5">
              <label className="block text-xs text-muted-foreground">Folder local</label>
              <span className="text-xs text-muted-foreground/50">optional</span>
              <Tooltip delayDuration={300}>
                <TooltipTrigger asChild>
                  <HelpCircle className="h-3 w-3 text-muted-foreground/50 cursor-help" />
                ...
                <TooltipContent side="top" className="max-w-[240px] text-xs">
                  Seteaza o cale absoluta pe aceasta masina unde agentii locali vor citi si scrie fisiere pentru acest proiect.
                </TooltipContent>
              </Tooltip>
            </div>
          ...
          <Popover open={goalOpen} onOpenChange={setGoalOpen}>
            <PopoverTrigger asChild>
              ...
              >
                ...
                {selectedGoals.length > 0 ? "+ Obiectiv" : "Obiectiv"}
              </button>
            </PopoverTrigger>
            <PopoverContent className="w-56 p-1" align="start">
              ...
              {selectedGoals.length > 0 && availableGoals.length === 0 && (
                <div className="px-2 py-1.5 text-xs text-muted-foreground">
                  Toate obiectivele sunt deja selectate.
                </div>
              )}
            </PopoverContent>
          </Popover>
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-t border-border">
          {createProject.isError ? (
            <p className="text-xs text-destructive">Nu s-a putut crea proiectul.</p>
          ) : (
            <span />
          )}
          <Button
          ...
          >
            {createProject.isPending ? "Se creeaza…" : "Creeaza proiect"}
          </Button>
        </div>
      </DialogContent>
    </Dialog>

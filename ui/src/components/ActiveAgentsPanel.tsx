    ...
    <div>
      ...
      {runs.length === 0 ? (
        <div className="rounded-xl border border-border p-4">
          <p className="text-sm text-muted-foreground">Nicio rulare recenta.</p>
        </div>
      ) : (
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            ...
            <div className="mt-2 flex items-center gap-2 text-[11px] text-muted-foreground">
              <span>{isActive ? "In direct" : run.finishedAt ? `Terminat ${relativeTime(run.finishedAt)}` : `Inceput ${relativeTime(run.createdAt)}`}</span>
            </div>
          </div>

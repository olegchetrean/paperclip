...
export function GoalDetail() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: "Obiective", href: "/goals" },
      { label: goal?.title ?? goalId ?? "Obiectiv" }
    ]);
      ...
      <Tabs defaultValue="children">
        <TabsList>
          <TabsTrigger value="children">
            Sub-Obiective ({childGoals.length})
          </TabsTrigger>
          <TabsTrigger value="projects">
            Proiecte ({linkedProjects.length})
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="children" className="mt-4 space-y-3">
          ...
          {childGoals.length === 0 ? (
            <p className="text-sm text-muted-foreground">Fara sub-obiective.</p>
          ) : (
        ...
        <TabsContent value="projects" className="mt-4">
          {linkedProjects.length === 0 ? (
            <p className="text-sm text-muted-foreground">Fara proiecte asociate.</p>
          ) : (

  ...
  return (
    <Card>
      <CardHeader className="px-5 pt-5 pb-2">
        <CardTitle className="text-base">Registru financiar</CardTitle>
        <CardDescription>
          Cheltuieli la nivel de cont care nu corespund unei singure cereri de inferenta.
        </CardDescription>
      </CardHeader>
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        <MetricTile
          ...
          subtitle={`${eventCount} eveniment${eventCount === 1 ? "" : "e"} in total in perioada`}
          icon={ArrowUpRight}
        />
        <MetricTile
          label="Creditari"
          value={formatCents(creditCents)}
          subtitle="Rambursari, compensari si retururi de credit"
          icon={ArrowDownLeft}
        />
        <MetricTile
          label="Net"
          value={formatCents(netCents)}
          subtitle="Debit minus credit pentru perioada selectata"
          icon={ReceiptText}
        />
        <MetricTile
          label="Estimat"
          value={formatCents(estimatedDebitCents)}
          subtitle="Debitari estimate care nu sunt inca confirmate pe factura"
          icon={Coins}
        />
      </CardContent>
    </Card>
...
export function Costs() {
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={DollarSign} message="Selecteaza o companie pentru a vedea costurile." />;
  }
    ...
    <div className="space-y-6">
      <div className="space-y-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                <h1 className="text-3xl font-semibold tracking-tight">Costuri</h1>
                <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
                  Cheltuieli de inferenta, taxe platforma, creditari si ferestre de cota live.
                </p>
            </div>
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              label="Cheltuieli inferenta"
              value={formatCents(spendData?.summary.spendCents ?? 0)}
              subtitle={`${formatTokens(inferenceTokenTotal)} tokeni in evenimente per cerere`}
              icon={DollarSign}
            />
            <MetricTile
              label="Buget"
              ...
              subtitle={
                activeBudgetIncidents.length > 0
                  ? `${budgetData?.pausedAgentCount ?? 0} agenti in pauza · ${budgetData?.pausedProjectCount ?? 0} proiecte in pauza`
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                    ? `${formatCents(spendData.summary.spendCents)} din ${formatCents(spendData.summary.budgetCents)}`
                    : "Niciun plafon lunar configurat"
              }
              icon={Coins}
            />
            <MetricTile
              ...
              subtitle={`${formatCents(financeData?.summary.debitCents ?? 0)} debitari · ${formatCents(financeData?.summary.creditCents ?? 0)} creditari`}
              icon={ReceiptText}
            />
            <MetricTile
              label="Evenimente financiare"
              value={String(financeData?.summary.eventCount ?? 0)}
              subtitle={`${formatCents(financeData?.summary.estimatedDebitCents ?? 0)} estimate in perioada`}
              icon={ArrowUpRight}
            />
          </div>
      </div>
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        <TabsList variant="line" className="justify-start">
          ...
          <TabsTrigger value="billers">Facturatori</TabsTrigger>
          <TabsTrigger value="finance">Financiar</TabsTrigger>
        </TabsList>

        <TabsContent value="overview" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">Selecteaza o data de inceput si una de sfarsit pentru a incarca datele.</p>
          ) : showOverviewLoading ? (
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.3fr,1fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">Registru inferenta</CardTitle>
                    <CardDescription>
                      Cheltuieli de inferenta per cerere pentru perioada selectata.
                    </CardDescription>
                  </CardHeader>
                    ...
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-sm text-muted-foreground">
                          {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                            ? `Buget ${formatCents(spendData.summary.budgetCents)}`
                            : "Buget nelimitat"}
                        </div>
                      </div>
                    ...
                    {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {spendData.summary.utilizationPercent}% din bugetul lunar consumat in aceasta perioada.
                        </div>
                      </div>
                    ) : null}
                  </CardContent>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.25fr,0.95fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    ...
                    <CardDescription>Ce a consumat fiecare agent in perioada selectata.</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-2 px-5 pb-5 pt-2">
                    {(spendData?.byAgent.length ?? 0) === 0 ? (
                      <p className="text-sm text-muted-foreground">Niciun eveniment de cost inca.</p>
                    ) : (
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">Per proiect</CardTitle>
                      <CardDescription>Costuri de rulare atribuite prin taskuri asociate proiectelor.</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
                      {(spendData?.byProject.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">Niciun cost de rulare atribuit proiectelor inca.</p>
                      ) : (
                        spendData?.byProject.map((row, index) => (
                          ...
                          >
                            <span className="truncate">{row.projectName ?? row.projectId ?? "Neatribuit"}</span>
                  ...
                  <FinanceTimelineCard rows={topFinanceEvents.slice(0, 6)} emptyMessage="Niciun eveniment financiar inca. Adauga cheltuieli la nivel de cont cand sosesc facturile sau creditarile." />
                </div>
              </div>
            </>
          )}
            ...
            <>
              <Card className="border-border/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))]">
                <CardHeader className="px-5 pt-5 pb-3">
                  <CardTitle className="text-base">Panou de control bugete</CardTitle>
                  <CardDescription>
                    Limite stricte de cheltuieli pentru agenti si proiecte. Cotele de abonament la furnizori raman separate si apar la Furnizori.
                  </CardDescription>
                </CardHeader>
                <CardContent className="grid gap-3 px-5 pb-5 pt-0 md:grid-cols-4">
                  <MetricTile
                    ...
                    subtitle="Depasiri de prag soft sau hard deschise"
                    icon={ReceiptText}
                  />
                  <MetricTile
                    label="Aprobari in asteptare"
                    value={String(budgetData?.pendingApprovalCount ?? 0)}
                    subtitle="Aprobari de depasire buget in asteptarea actiunii"
                    icon={ArrowUpRight}
                  />
                  <MetricTile
                    label="Agenti in pauza"
                    value={String(budgetData?.pausedAgentCount ?? 0)}
                    subtitle="Heartbeat-uri agenti blocate de buget"
                    icon={Coins}
                  />
                  <MetricTile
                    label="Proiecte in pauza"
                    value={String(budgetData?.pausedProjectCount ?? 0)}
                    subtitle="Executie proiecte blocata de buget"
                    icon={DollarSign}
                  />
                </CardContent>
              </Card>
              ...
              {activeBudgetIncidents.length > 0 ? (
                <div className="space-y-3">
                  <div>
                    <h2 className="text-lg font-semibold">Incidente active</h2>
                    <p className="text-sm text-muted-foreground">
                      Rezolva blocajele aici prin cresterea bugetului sau mentinerea explicita a pauzei.
                    </p>
                  </div>
                  <div className="grid gap-4 xl:grid-cols-2">
                ...
                {(["company", "agent", "project"] as const).map((scopeType) => {
                  ...
                  return (
                    <section key={scopeType} className="space-y-3">
                      <div>
                        <h2 className="text-lg font-semibold capitalize">{scopeType === "company" ? "Bugete companie" : scopeType === "agent" ? "Bugete agenti" : "Bugete proiecte"}</h2>
                        <p className="text-sm text-muted-foreground">
                          {scopeType === "company"
                            ? "Politica lunara la nivel de companie."
                            : scopeType === "agent"
                              ? "Politici lunare recurente de cheltuieli pentru agenti individuali."
                              : "Politici de cheltuieli pe viata pentru proiecte legate de executie."}
                        </p>
                      </div>
                      <div className="grid gap-4 xl:grid-cols-2">
                        {rows.map((summary) => (
                ...
                {budgetPolicies.length === 0 ? (
                  <Card>
                    <CardContent className="px-5 py-8 text-sm text-muted-foreground">
                      Nicio politica de buget inca. Seteaza bugete pentru agenti si proiecte din paginile lor de detalii, sau foloseste controlul lunar de buget al companiei.
                    </CardContent>
                  </Card>
                ) : null}
              </div>
            ...
            <>
              <Tabs value={effectiveProvider} onValueChange={setActiveProvider}>
                ...
                <TabsContent value="all" className="mt-4">
                  {providers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">Niciun eveniment de cost in aceasta perioada.</p>
                  ) : (
          ...
          {showCustomPrompt ? (
            ...
            <>
              <Tabs value={effectiveBiller} onValueChange={setActiveBiller}>
                ...
                <TabsContent value="all" className="mt-4">
                  {billers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">Niciun eveniment facturabil in aceasta perioada.</p>
                  ) : (
                    <div className="grid gap-4 md:grid-cols-2">
                      {billers.map((biller) => {
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.2fr,0.95fr]">
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">Per facturator</CardTitle>
                      <CardDescription>Evenimente financiare la nivel de cont grupate dupa cine a facturat sau creditat.</CardDescription>
                    </CardHeader>
                    <CardContent className="grid gap-4 px-5 pb-5 pt-2 md:grid-cols-2">
                      {(financeData?.byBiller.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">Niciun eveniment financiar inca.</p>
                      ) : (

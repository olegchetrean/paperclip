...
export function PluginSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: "Setari", href: "/instance/settings/heartbeats" },
      { label: "Pluginuri", href: "/instance/settings/plugins" },
      { label: plugin?.manifestJson?.displayName ?? plugin?.packageName ?? "Detalii Plugin" },
    ]);
  ...
  if (pluginLoading) {
    return <div className="p-4 text-sm text-muted-foreground">Se incarca detaliile pluginului...</div>;
  }
  ...
  const statusVariant =
    plugin.status === "ready"
      ...
      : plugin.status === "error"
        ? "destructive"
        : "secondary";
  const pluginDescription = plugin.manifestJson.description || "Nicio descriere furnizata.";
    ...
    <div className="space-y-6 max-w-5xl">
      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as "configuration" | "status")} className="space-y-6">
        <PageTabBar
          align="start"
          items={[
            { value: "configuration", label: "Configurare" },
        ...
        <TabsContent value="configuration" className="space-y-6">
          <div className="space-y-8">
            <section className="space-y-5">
              <h2 className="text-base font-semibold">Despre</h2>
              <div className="grid gap-8 lg:grid-cols-[minmax(0,1.4fr)_minmax(220px,0.8fr)]">
                ...
                <div className="space-y-4 text-sm">
                  ...
                  <div className="space-y-2">
                    <h3 className="font-medium text-muted-foreground">Categorii</h3>
                    <div className="flex flex-wrap gap-2">
                      {plugin.categories.length > 0 ? (
                        ...
                        <span className="text-foreground">Niciuna</span>
                      )}
                    </div>
                  </div>
                </div>
            ...
            <section className="space-y-4">
              <div className="space-y-1">
                <h2 className="text-base font-semibold">Setari</h2>
              </div>
              {hasCustomSettingsPage ? (
                ...
                <p className="text-sm text-muted-foreground">
                  Acest plugin nu necesita nicio setare.
                </p>
              )}
            </section>
          </div>
          ...
          <div className="grid gap-6 xl:grid-cols-[minmax(0,1.2fr)_320px]">
            <div className="space-y-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Proces worker, joburi planificate si livrari webhook
                  </CardDescription>
                </CardHeader>
                    ...
                    <>
                      <div>
                        ...
                        {dashboardData.worker ? (
                          ...
                          <p className="text-sm text-muted-foreground italic">Niciun proces worker inregistrat.</p>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        {dashboardData.recentJobRuns.length > 0 ? (
                          ...
                          <p className="text-sm text-muted-foreground italic">Nicio rulare de job inregistrata inca.</p>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        {dashboardData.recentWebhookDeliveries.length > 0 ? (
                          ...
                          <p className="text-sm text-muted-foreground italic">Nicio livrare webhook inregistrata inca.</p>
                        )}
                      </div>

                      <div className="flex items-center gap-1.5 border-t border-border/50 pt-2 text-xs text-muted-foreground">
                        <Clock className="h-3 w-3" />
                        Ultima verificare: {new Date(dashboardData.checkedAt).toLocaleTimeString()}
                      </div>
                    </>
                  ) : (
                    <p className="text-sm text-muted-foreground">
                      Diagnosticele runtime nu sunt disponibile momentan.
                    </p>
                  )}
                </CardContent>
              </Card>
              ...
              {recentLogs && recentLogs.length > 0 ? (
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>Ultimele {recentLogs.length} intrari de log</CardDescription>
                  </CardHeader>
                  <CardContent>
            ...
            <div className="space-y-6">
              <Card>
                ...
                <CardContent>
                  {healthLoading ? (
                    <p className="text-sm text-muted-foreground">Se verifica sanatatea...</p>
                  ) : healthData ? (
                    <div className="space-y-4 text-sm">
                    ...
                    <div className="space-y-3 text-sm text-muted-foreground">
                      ...
                      <p>Verificarile de sanatate ruleaza odata ce pluginul este gata.</p>
                      {plugin.lastError ? (
                        <div className="break-words rounded border border-destructive/20 bg-destructive/10 p-2 text-xs text-destructive">
                          {plugin.lastError}
                        </div>
              ...
              <Card>
                ...
                <CardContent>
                  {pluginCapabilities.length > 0 ? (
                    ...
                    <p className="text-sm text-muted-foreground italic">Nicio permisiune speciala solicitata.</p>
                  )}
                </CardContent>
              </Card>
            </div>
...
function PluginConfigForm({ pluginId, schema, initialValues, isLoading, pluginStatus, supportsConfigTest }: PluginConfigFormProps) {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      setSaveMessage({ type: "success", text: "Configurare salvata." });
      setTestResult(null);
    ...
    onError: (err: Error) => {
      setSaveMessage({ type: "error", text: err.message || "Nu s-a putut salva configurarea." });
    },
  });
  ...
  const testMutation = useMutation({
    ...
    onSuccess: (result) => {
      if (result.valid) {
        setTestResult({ type: "success", text: "Testul configurarii a trecut." });
      } else {
        setTestResult({ type: "error", text: result.message || "Testul configurarii a esuat." });
      }
    },
    onError: (err: Error) => {
      setTestResult({ type: "error", text: err.message || "Testul configurarii a esuat." });
    },
  });
